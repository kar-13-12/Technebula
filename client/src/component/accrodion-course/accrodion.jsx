...
function MyComponent() {
    ...
    const biologyImages = [bioImage1, bioImage2,bioImage3,bioImage4,bioImage5,bioImage6,bioImage7,bioImage8,bioImage9,bioImage10]; // List of biology images
    const physicsImages = [physicsImage1, physicsImage2, physicsImage3,physicsImage4,physicsImage5,physicsImage6,physicsImage7,physicsImage8,physicsImage9,physicsImage10,physicsImage11,physicsImage12,physicsImage13,physicsImage14,physicsImage15,physicsImage16,physicsImage17,physicsImage18,physicsImage19,physicsImage20]; // List of physics images
    const chemistryImages = [chemistryImage1, chemistryImage2, chemistryImage3,chemistryImage4,chemistryImage5,chemistryImage6,chemistryImage7,chemistryImage8,chemistryImage9,chemistryImage10,chemistryImage11,chemistryImage12,chemistryImage13,chemistryImage14,chemistryImage15,chemistryImage16,chemistryImage17,chemistryImage18,chemistryImage19,chemistryImage20]; // List of chemistry images
    const mathImages = [mathImage1,mathImage2,mathImage3,mathImage4,mathImage5,mathImage6,mathImage7,mathImage8,mathImage9,mathImage10,mathImage11,mathImage12,mathImage13,mathImage14]
    ...
    const handleNextClick = () => {
        setSelectedTopicIndex((selectedTopicIndex + 1) % biocourses[selectedCourseIndex].topics.length);
    };

    const handlePreviousClick = () => {
        setSelectedTopicIndex((selectedTopicIndex - 1 + biocourses[selectedCourseIndex].topics.length) % biocourses[selectedCourseIndex].topics.length);
    };
  ...
  <div className='course-title' style={{ display: 'flex', alignItems: 'center', marginBottom: '20px' }}>
    <div className='style' style={{ marginRight: '100px' }}>
      {biocourses.map((course, courseIndex) => (
        <div key={courseIndex} style={{ marginBottom: '20px' }}>
          <Accordion expanded={openAccordion === course.title} onChange={handleAccordionChange(course.title)}>
            <AccordionSummary
              ...
              <div>
                {course.topics.map((topic, topicIndex) => (
                  <Button
                    ...
                    color="primary"
                    style={{ marginBottom: '10px', display: 'block', color: 'black' }}
                  >
                    {topic}
                  </Button>
                ))}
    ...
    <div className='bio-img' style={{ position: 'relative' }}>
      ...
      />
      {selectedTopicIndex < biocourses[selectedCourseIndex].topics.length - 1 && (
        <img
        className='img-course'
          ...
          style={{ width: '200px', position: 'absolute', top: 0, left: '100%', marginLeft: '20px' }} // Adjust width and margins as needed
        />
      )}
    </div>
  </div>
  <div style={{ marginBottom: '20px' }}>
    <Button onClick={handlePreviousClick} variant="contained" color="primary" style={{ marginRight: '10px' }}>Previous</Button>
    <Button onClick={handleNextClick} variant="contained" color="primary">Next</Button>
  </div>
</div>
